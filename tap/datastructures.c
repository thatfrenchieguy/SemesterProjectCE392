...
singly and doubly linked lists, binary heaps, queues, as well as memory allocation
and deallocation.
...
linkedListElt *insertLinkedList(linkedList *list, int value, linkedListElt *after) {
...
void displayLinkedList(int minVerbosity, linkedList *list) {
	...
	while (curnode != NULL) {
		displayMessage(minVerbosity, "%p: %d -> %p\n", (void *)curnode, curnode->value, (void *)curnode->next);
...
doublyLinkedListElt *insertDoublyLinkedList(doublyLinkedList *list, double value, doublyLinkedListElt *after) {
...
void deleteDoublyLinkedListElt(doublyLinkedList *list, doublyLinkedListElt *elt) {
	if (list->tail != elt) {
...
void displayDoublyLinkedList(int minVerbosity, doublyLinkedList *list) {
	...
	displayMessage(minVerbosity, "Start of the list: %p\n", (void *)list->head);
	while (curnode != NULL) {
		displayMessage(minVerbosity, "%p %f %p %p\n", (void *)curnode, curnode->value, (void *)curnode->prev, (void *)curnode->next);
		curnode = (*curnode).next;
	}
	displayMessage(minVerbosity, "End of the list: %p\n", (void *)list->tail);
}
...
void frontQueue(queue_type *queue, long elt) {
	if (queue->history[elt] == IN_QUEUE) return;
	if (queue->readptr == 0) queue->readptr = queue->size - 1; else queue->readptr--;
	if (queue->curelts == queue->size) fatalError("Queue not large enough!");
	queue->curelts++;
...
void displayQueue(int minVerbosity, queue_type *queue) {
	...
	for (i = 0; i < queue->size; i++) {
		...
		if (i == queue->readptr) displayMessage(minVerbosity, "R"); else displayMessage(minVerbosity, " ");
		if (i == queue->writeptr) displayMessage(minVerbosity, "W"); else displayMessage(minVerbosity, " ");
...
void siftUp(heap_type *heap, int elt) {
	while (elt > 0 && heap->valueFn[heap->node[elt]] < heap->valueFn[heap->node[heapPred(elt)]]) {
		swap(heap->nodeNDX[heap->node[elt]], heap->nodeNDX[heap->node[heapPred(elt)]]);
...
void siftDown(heap_type *heap, int elt) {
	int tmp;
	while (heapSucc(elt) <= heap->last && heap->valueFn[heap->node[elt]] > heap->valueFn[heap->node[minChild(heap, elt)]]) {
...
int minChild(heap_type *heap, int elt) {
	if (heapSucc(elt) == heap->last)
		return heap->last;
	if (heap->valueFn[heap->node[heapSucc(elt)]] <= heap->valueFn[heap->node[heapSucc(elt) + 1]])
		return heapSucc(elt);
...
void displayHeap(int minVerbosity, heap_type *heap) {
	int i;
	displayMessage(minVerbosity, "HEAP current size, capacity, number of elements: %d %d %d\n", heap->last, heap->maxsize, heap->maxelts);
...
void *allocateScalar(size_t size) {
	...
	#ifdef MEMCHECK
	memcheck_numScalars++;
	if (memcheck_numScalars > MEMCHECK_THRESHOLD) { displayMessage(DEBUG, "Now have %ld scalars.\n", memcheck_numScalars); }
	#endif
	return scalar;
}
...
void *allocateVector(long u, size_t size) {
	...
	if (vector == NULL) fatalError("Unable to allocate memory for vector of size %ld.", u);
	#ifdef MEMCHECK
	memcheck_numVectors++;
	if (memcheck_numVectors > MEMCHECK_THRESHOLD) { displayMessage(DEBUG, "Now have %ld vectors.\n", memcheck_numVectors); }
	#endif
	return vector;
}
...
void **allocateMatrix(long u1, long u2, size_t size) {
	...
	if (matrix == NULL) fatalError("Unable to allocate memory for matrix of size %ld x %ld.", u1, u2);
	for (i = 0; i < u1; i++) {
		matrix[i] = malloc(u2 * size);
		if (matrix[i] == NULL) fatalError("Unable to allocate memory for matrix of size %ld x %ld.", u1, u2);
	}
	#ifdef MEMCHECK
	memcheck_numMatrices++;
	if (memcheck_numMatrices > MEMCHECK_THRESHOLD) { displayMessage(DEBUG, "Now have %ld matrices.\n", memcheck_numMatrices); }
	#endif
	return matrix;
}
...
void ***allocate3DArray(long u1, long u2, long u3, size_t size) {
	long i, j;
	void ***matrix = malloc(u1 * sizeof(void **));
	if (matrix == NULL) fatalError("Unable to allocate 3D array of size %ld x %ld x %ld.", u1, u2, u3);
	for (i = 0; i < u1; i++) {
		matrix[i] = malloc(u2 * sizeof(void *));
		if (matrix[i] == NULL) fatalError("Unable to allocate 3D array of size %ld x %ld x %ld.", u1, u2, u3);
		for (j = 0; j < u2; j++) {
			matrix[i][j] = malloc(u3 * size);
			if (matrix[i][j] == NULL) fatalError("Unable to allocate 3D array of size %ld x %ld x %ld.", u1, u2, u3);
		}
	}
	#ifdef MEMCHECK
	memcheck_num3DArrays++;
	if (memcheck_num3DArrays > MEMCHECK_THRESHOLD) { displayMessage(DEBUG, "Now have %ld 3D arrays.\n", memcheck_num3DArrays); }
	#endif
	return matrix;
}
