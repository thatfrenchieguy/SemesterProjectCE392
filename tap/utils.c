...
/*
SWAP is a generic swap function using void pointers to exchange arbitrary memory.
*/
...
/*
updateElapsedTime helps with timing, adding an increment to elapsedTime.
Note that this function does not reset startTime (in order to allow for calculations
to be selectively excluded from timing, e.g. when generating logs or debug data).  You
need to separately set startTime once you want to begin recording.
*/
...
/*
displayMessage is a general-purpose printing function.  If the (global) verbosity
variable is high enough (exceeds the minVerbosity argument), then prints the message
indicated.  From least to greatest, verbosity levels are:
	NOTHING
	LOW_NOTIFICATIONS
	...
	DEBUG
	FULL_DEBUG
Output at the DEBUG and FULL_DEBUG levels is not printed to the screen; rather, if
DEBUG_MODE is enabled (by defining the appropriate preprocessor macro) these messages
will be written to the debug log (typically debug.txt)
*/
